...
c        cube length is maximum distance at which two SAS could intersect
         ...
         !write(6,*) "I AM HERE (3.0) " 
         !flush(6)
         !write (6,*) 4*(cb(1)+1)*(cb(2)+1)*(cb(3)+1)
         !flush(6)
         !write (6,*) (4*27*anum)
         !flush(6)
         !write (6,*) "i_cblower"
         ...
         !i_cbatom  = memalloc(i_cbatom,100000)
         !i_cbatom  = memalloc(i_cbatom,(4*27*anum))
         !write (6,*) "i_cbatom"
         !flush(6)
         i_cbatom  = memalloc(i_cbatom,(4*27*anum))
         !i_cbatom = memalloc(i_cbatom,10*4*27*anum)
         !write (6,*) 'anum = ', anum
         !write (6,*) i_cbatom
         !flush (6)
         !write (6,*)  cbatom(1)
...
c        local dynamic arrays
         ...
         !real*4 radp2(1), radcheck(1)
         real radp2(anum), radcheck(anum)
         integer recptr(4*anum), inaccptr(4*anum)
         !real*4 ligpts(3,1)
         real ligpts(3,numpts)
...
c        main loops over all possible grid positions of the ligand atom
         do k = 0, n(3)
          ...
          write(6,'(i4,a,i4,a)') k+1,' / ',n(3)+1,' the way there!'
          call flush(6)
