...
static void pfind(FILE *file, int newparm, char *label)
{
  ...
  if (get_mytaskid() == 0) {
    fprintf( nabout,"Error: failed to find prmtop section %s\n", label );
  }
  reducerror(-1);
...
int readparm(MOLECULE_T  *mol, char *name)
{
  ...
  if (get_mytaskid() == 0) {
    fscanf(file, "%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d",
...
#define BOFFSET 0.09
  if( newparm ){
    ...
    if (get_mytaskid() == 0) {
      fprintf( nabout, "old prmtop format => using old algorithm for GB parms\n" );
    }
    prm->Fsmax = 0.0;
    for (i = 0; i < prm->Natom; i++) {
...
int  writeparm(MOLECULE_T *mol, char *name)
{
  ...
  if (get_mytaskid() == 0) {
	...
	tmpchar[2] = tmpchar[3] = ' '; tmpchar[4] = '\0';
	FortranWriteString(tmpchar);
      }
      FortranEndLine();
