 ...
 *
 * Main interface is through routines "mme", "mme4", "mme_init",
 * "mme_rattle" and "md".
 *
...
/* If STATIC_ARRAYS is defined, allocate large arrays once only. */
...
REAL_T tnewton, tlevel, tcholesky, tdiag, tmmetwo, tnmode, tconjgrad, tmmeone;
...
static REAL_T tb2g1, tb2g2, tb2g3, tloop;
...
static REAL_T cut2;                     /* non-bonded cutoff squared */
static int ntpr = 10;          /* print frequency */
static int nsnb = 10;            /* non-bonded update frequency */
static int nchk = 10000, nchk2 = 10000; /* checkpoint update frequencies */
...
static int gb_debug = 0;     /* non-zero if extra printout for gen. Born  */
static int nr_debug = 0;     /* non-zero if extra printout for Newt. Raph. */
static int gb2_debug = 0;     /* non-zero if extra printout for gen. Born  */
...
static int *ineighbor;                 /* neighborlist-array */
    /* 34 seems sufficient. this should, however, be changed */
    /* if it proves unsafe */
static const int INEIGHBOR_SIZE = 73;  /* neighborlist-array size */
static int gbsa = 0;
static REAL_T surften = 0.005;    /* 'surface tension' for GB/SA */
static REAL_T esurf = 0.0;        /* nonpolar solvation */
...
static REAL_T boltz2 = 9.93595e-4;
...
#ifdef MORT
int set_belly_mask ( int, char *, int * );
int set_cons_mask ( int, char *, int * );
#else
...
REAL_T gauss( REAL_T *, REAL_T *, int * );
...
int getxyzw( char **fname, int *natom, REAL_T *x, int dim )
{
  ...
  for( i=0; i<dim*(*natom); i++ ){
    strncpy( field, line + 20*(i%dim), 20 ); field[20] = '\0';
    x[i] = atof( field );
...
int getxv( char **fname, int *natom, REAL_T *start_time, REAL_T *x, REAL_T *v )
{
  FILE *fp;
  int i, ier;
  ...
  *start_time = 0.0; /* default, in case the next line does not assign it */
  ...
  ggets( line, sizeof( line ), fp );
  for( i=0; i<3*(*natom); i++ ){
    strncpy( field, line + 12*(i%6), 12 ); field[12] = '\0';
    x[i] = atof( field );
    if( i%6 == 5 && i<3*(*natom)-1 ) ggets( line, sizeof( line ), fp );
  }
  ...
  if ( ggets( line, sizeof( line ), fp ) ){
    for( i=0; i<3*(*natom); i++ ){
      strncpy( field, line + 12*(i%6), 12 ); field[12] = '\0';
      v[i] = atof( field );
      if( i%6 == 5 ) ggets( line, sizeof( line ), fp );
    }
  } else {
    for( i=0; i<3*(*natom); i++ ) v[i] = 0.0;
 ...
 * in that eff.c should preceed sff2.c.  Also, they should follow the
              ...
              REAL_T *x0i, FILE *bfpi )
{
  ...
  if (gbsa == 1) {
    ...
    ineighbor = ivector(0, INEIGHBOR_SIZE * prm->Natom);
    ...
    for (i = 0; i < prm->Natom; i++) {
      ...
      if (strncmp(atypi, "CT", 2) == 0) {
        if (numb[i] == 1) {
		  ...
		  fprintf( nabout, "bad number of bonds to CT: %d %d; ", i, numb[i] );
        ...
        if (numb[i] == 2) {
		  ...
		  fprintf( nabout, "bad number of bonds to C: %d %d; ", i, numb[i] );
		  fprintf( nabout, "using default carbon parameters\n" );
          P0[i] = 1.70 + 1.4;
          P1[i] = 0.51245;
          P2[i] = -0.15966;
        ...
        if (numb[i] == 1) {
		  ...
		  fprintf( nabout, "bad number of bonds to O*: %d %d; ", i, numb[i] );
        ...
        if (numb[i] == 1) {
		  ...
		  fprintf( nabout, "bad number of bonds to N3: %d %d; ", i, numb[i] );
        ...
        if (numb[i] == 1) {
		  ...
		  fprintf( nabout, "bad number of bonds to N: %d %d; ", i, numb[i] );
		  fprintf( nabout, "using default nitrogen parameters\n" );
          P0[i] = 1.65 + 1.4;
          P1[i] = 0.73511;
          P2[i] = -0.22116;
        ...
        if (numb[i] == 3) {
		  ...
		  fprintf( nabout, "bad number of bonds to P: %d %d; ", i, numb[i] );
      ...
      if( P0[i]>5.0 ) fprintf(nabout, "bad p0: %d %12.6f %s\n", i, P0[i], atypi );
    }
...
#ifdef MORT
  nconstrained = set_cons_mask(prm->Natom, aexp2, constrained);
#else
  ...
  if( gb==2 ){
    gbalpha = 0.8; gbbeta = 0.0; gbgamma = 2.909125;
  } else if ( gb==5 ){
    gbalpha = 1.0; gbbeta = 0.8; gbgamma = 4.85;
  }

#if defined(SCALAPACK) || defined(MPI)

  /*
   * Free (if allocated) then reallocate the pairlist2 and pair count arrays.
   * The pair list array is an array of arrays, and could contain NULL
  ...
  /*
   * Free (if allocated) then reallocate the pairlist and pair count arrays.
   * The pair list array is an array of arrays, and could contain NULL
   * elements if mme_init were called a second time prior to initialization
   * of the pair lists by nblist.
   */
...
#ifdef MORT
  nfrozen = set_belly_mask(prm->Natom, aexp, frozen);
#else
  nfrozen = set_belly_mask(m, aexp, frozen);
#endif
  if (nfrozen) {
    if (mytaskid == 0) {
      fprintf(nabout, "freezing %d atoms using expression %s for moving atoms\n",
         nfrozen, aexp);
    }
  }
  ...
  if (JexclAt != NULL) {
    ...
    if (Jblo != NULL) {
      ...
      fprintf(nabout, "Error deallocating JexclAt[%d] in mme_init!\n", j);
      fflush(nabout);
      mpierror(-1);
    }
      }
  ...
  if ( (Jatoms = (ATOMSTR **) malloc(prm->Natom * sizeof(ATOMSTR *))) == NULL) {
  ...
  tconjgrad = tmd = tcholesky = tnewton = tlevel = tdiag = 0.0;
  tmmeone = tmmetwo = tb2g1 = tb2g2 = tb2g3 = tloop = 0.0;

  /*  if the size has grown, call mme34() to free up the storage:  */
  i = -3;
  if( prm->Natom > nold ) mme34( x0i, x0i, &i );
...
#define EPS 0.0000001
  ...
  for (i = 0; i < nat3 / 3; i++) {
    tmpf =
      f[3 * i] * f[3 * i] + f[3 * i + 1] * f[3 * i + 1] + f[3 * i + 2] *
      f[3 * i + 2];
    frms += tmpf;
...
{
  REAL_T dtx, dt5, rndf, dttp, ekin0, ekin, epot, etot, tscal, temp, sd;
  REAL_T gammai, c_implic, c_explic, sdfac, zero, invmass, t1, t2;
  int nstep, i;

  static REAL_T *xold=NULL, *sqrmass=NULL, *accel=NULL, *minv=NULL;
  ...
  dt5 = 0.5 * dtx;
  rndf = n - 3 * nfrozen;
  if( irattle ) rndf -= prm->Nbonh + prm->Mbona;
  ...
  if( gammai == 0.0 ){  
    ...
    for (i = 0; i < n; i++){
      if( frozen[ i/dim ] ){
        ...
        sd = sdfac*sqrmass[i];
        accel[i] = (-f[i] + gauss(&zero, &sd, &idum)) * minv[i] * dt5;
      }
    }
  }
  ...
  for (nstep = 1; nstep <= maxstep; nstep++) {
    ...
    if( irattle ) rattle( dtx, x, xold, v, minv );
    ...
    if( irattle ) rattle2( dtx, x, v, minv );
