...
// add an option to GA through all the new fragments you just built at the appropriate place in the code below (no additions or deletions, just cross-overs with occasional mutations) 
            ...
            // Keeping track of how many molecules failed to write to file on lower bound mw cutoff. If dn_lower_constraint_mol_wt = 0 do not bother with this statement.
            if (dn_ga_flag == false && (dn_lower_constraint_mol_wt > 0)){
               cout << "      While writing molecules to file, " << mw_low_prune_counter << " molecules were rejected on the lower bound molecular weight." << endl;
            }
