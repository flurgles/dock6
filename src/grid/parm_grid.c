...
*/
  //hack to get rid of chemical scoring function call
  label->chemical.flag = FALSE;
  get_parameter
  (
  ...
  //Flag added for whether residues with a non integer charge should be allowed
  get_parameter
  (
    (void *) &score_energy->non_integral_charges,
     &parm, Boolean, "allow_non_integral_charges",
     score_energy->flag ? "YES no" : "NO",
     score_energy->flag
  );
