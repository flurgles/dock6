...
#Trent E Balius update to python3, 2020
...
keepPolarH = {
    # H   ALA
    #'ALA': ('H  '),
    'ALA': ('H  ',terminalH[0],terminalH[1],terminalH[2]),
    # H   ARG
    # HE  ARG
    #HH11 ARG
    #HH12 ARG
    ...
    'ARG': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HE ', 'HH11', 'HH12', 'HH21', 'HH22'),
    # H   ASN
    #HD21 ASN
    #HD22 ASN
    #'ASN': ('H  ', 'HD21', 'HD22'),
    'ASN': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HD21', 'HD22'),
    # H   ASP
    #'ASP': ('H  '),
    'ASP': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    # H   CYS
    # HG  CYS
    #'CYS': ('H  ', 'HG '),
    'CYS': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HG '),
    # H   CYX
    'CYX': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    'CYM': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    # H   GLN
    #HE21 GLN
    #HE22 GLN
    'GLN': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HE21', 'HE22'),
    # H   GLU
    'GLU': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    # H   GLY
    'GLY': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    # H   HIS
    # HD1 HIS (change to HID if only)
    # HE2 HIS (change to HIE if only) (if both change to HIP)
    'HIS': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HD1', 'HE2'),
    'HIP': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HD1', 'HE2'),
    'HIE': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HE2'),
    'HID': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HD1'),
    # H   ILE
    'ILE': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    # H   LEU
    'LEU': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    # H   LYS
    # HZ1 LYS
    # HZ2 LYS
    # HZ3 LYS
    'LYS': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HZ1', 'HZ2', 'HZ3'),
    # H   MET
    'MET': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    # H   PHE
    'PHE': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
    #nothing for proline
    #'PRO': (),
    'PRO': ( terminalH[0], terminalH[1], terminalH[2] ), 
    # H   SER
    # HG  SER
    'SER': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HG '),
    # H   THR
    # HG1 THR
    'THR': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HG1'),
    # H   TRP
    # HE1 TRP
    'TRP': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HE1'),
    # H   TYR
    # HH  TYR
    'TYR': ('H  ', terminalH[0], terminalH[1], terminalH[2], 'HH '),
    # H   VAL
    'VAL': ('H  ', terminalH[0], terminalH[1], terminalH[2]),
...
class pdbData(object):
  ...
  def processLine(self, line, modelNumber=0, radiiToUse=radiiDefault):
    ...
    #if (not ("ATOM" in line.split()[0]) or not ("HETATM" in line.split()[0])):
  ...
  def getOrderedRawIndices(self):
    ...
    indices = list(self.atomToRaw.values())
    indices.sort()
    return indices
  ...
  def setRadiiToFactors(self):
    ...
    for rawDataIndex in range(len(self.rawData)):
      newFactors = (
  ...
  def getListResiduesChains(self, residueChainNumbers):
    ...
    for index, thisResNum in enumerate(newPdb.resNums):
      ...
      thisCheck = ('+' + str(thisResNum) + str(chain)).strip() + '+'
  ...
  def removeApolarHydrogen(self):
    ...
    for atomIndex, atom in enumerate(self.atoms):
      if atom[0] == 'H':  # hydrogen atom
        ...
        if resName not in list(keepPolarH.keys()):
  ...
  def renameHistidines(self):
    ...
    for rawDataIndex in range(len(self.rawData)):
      if self.resNames[rawDataIndex] == "HIS":  # only care about unchanged his
        ...
        if chainResNum not in list(residueSets.keys()):
    ...
    #for indexList in residueSets.items():
    for indexList in iter(list(residueSets.values())):
  ...
  def renameCysteines(self):
    ...
    for rawDataIndex in range(len(self.rawData)):
      if self.resNames[rawDataIndex] == "CYS":  # only care about unchanged his
        #print(rawDataIndex, self.resNames[rawDataIndex],self.chains[rawDataIndex], self.resNums[rawDataIndex])
        chainResNum = (self.chains[rawDataIndex], self.resNums[rawDataIndex])
        if chainResNum not in list(residueSets.keys()):
          residueSets[chainResNum] = []
        residueSets[chainResNum].append(rawDataIndex)
    #second pass, decide whether each CYS is CYS or CYX
    for key in list(residueSets.keys()):
      #print (key)
      cyx = True
      indexList = residueSets[key]
      #print (indexList)
      for rawDataIndex in indexList:
        #print (rawDataIndex)
  ...
  def renumberResidues(self):
    ...
    for chainId in list(chainsHere.keys()):
    ...
    for index, coord in enumerate(self.coords):
      ...
      for data in otherData:
        try:
          ...
          for sumIndex in range(len(sums)):
  ...
  def getResidueNamesChains(self):
    ...
    for index in range(len(self.resNames)):
      residueName = self.resNames[index]
      residueNumber = self.resNums[index]
      chain = self.chains[index]
      resChain = str(residueName) + str(residueNumber) + str(chain)
  ...
  def getFasta(self):
    ...
    for index in range(len(self.resNums)):
      residueName = self.resNames[index]
      residueNumber = self.resNums[index]
      chain = self.chains[index]
      resChain = str(residueNumber) + str(chain)
  ...
  def getOccupancyResidue(self, resnum):
    ...
    for atomCount in range(len(self.atoms)):
  ...
  def selectMostOccupied(self, exceptions=[], leaveAlone=[]):
    ...
    #for atomTruple, atomList in sameAtoms.items():
    for atomTruple, atomList in iter(list(sameAtoms.items())):
  ...
  def alignOntoThis(self, otherPdb):
    ...
    for chainResNum, indexList in iter(list(otherResSets.items())):
  ...
  def getXYZatomsMatching(self, atomNames=None):
    ...
    xyzs = []
    for atomIndex, atom in enumerate(self.atoms):
      if atomNames is None or atom.strip() in atomNames:
  ...
  def alignOntoThisWholeProtein(self, otherPdb, atomNames=['CA']):
    ...
    try:
      ...
      for newIndex in range(len(newPdb.rawData)):
        newCoord = newCoords.pop(0)
        newPdb.updateNewCoord(newIndex, newCoord)
    except numpy.linalg.linalg.LinAlgError:
      print(('error', numpy.linalg.linalg.LinAlgError, chainResNum))
...
def writeLigandPdb(outName, atomXyzList, atomTypeList, radii, atomCharge):
  ...
  for atomCount, atomXyz in enumerate(atomXyzList):  # write atoms to pdb file
    ...
    for oneSpace in range(3 - len(atomType)):
      line += ' '
    line += ' LIG     1     '
