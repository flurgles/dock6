...
class AtomConverter(object):
  ...
  def __init__(self, parameterFileName=None):
    ...
    self.specialKeys = {}
    for aKey in list(self.convertTypes.keys()):
      #if -1 != string.find(aKey, '-'):  # aKey has a - in it
      #  index = string.find(aKey, '-')
      if -1 != aKey.find('-'):  # aKey has a - in it
        index = aKey.find('-')
  ...
  def printParameters(self):
    ...
    atomkeys = list(self.convertTypes.keys())
    #atomkeys.sort()
    sorted(atomkeys)
    for key in atomkeys:
  ...
  def convertMol2atomNum(self, mol2data, atomNum):
    ...
    if actualName not in list(self.specialKeys.values()):  # don't worry about bonds
      ...
      for key in iter(list(self.specialKeys.keys())):  # check each special bond
...
if -1 != sys.argv[0].find("sybyl2dock.py"):
