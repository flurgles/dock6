 ...
 #awk 'BEGIN{count=0}{if(count==0){count=1}else{print $0}}' prot-taut.info | sort -r -n -k2 | awk '{print $1 " " $2}' > prot-taut2.info
  ...
  foreach  dir (`cat ${mountdir}/dirlist`)
     ...
     foreach mol2 (`ls ${dir}_*[0123456789].mol2`)
       ...
       cd $db2dir
       if  ($#argv == 1) then 
          csh ${DOCK6BASE}/template_pipeline/hdb_lig_gen/generate/dock6_confgen_db2.csh ../${name}_output.mol2 ../${name}_output.solv ../${name}.name.txt 1000 1000
       else if  ($#argv == 2) then 
          ...
          #csh ${DOCK6BASE}/template_pipeline/hdb_lig_gen/generate/dock6_confgen_db2_covalent.csh ../${name}_output.mol2 ../${name}_output.solv ../${name}.name.txt 1000 1000
          csh ${DOCK6BASE}/template_pipeline/hdb_lig_gen/generate/dock6_confgen_db2_covalent.csh ../${name}_output.mol2 ../${name}_output.solv ../${name}.name.txt 10000000 10000000 
       endif
     end
  end
