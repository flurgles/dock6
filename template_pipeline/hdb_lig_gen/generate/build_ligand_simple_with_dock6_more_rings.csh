...
set DOCK6BASE = ${DOCK_HOME}
  ...
  echo " this script takes 3 arguments smiles file, number of conformations to generate, energy cutoff"
  ...
  #echo "I AM HERE"
  echo "filename = $1"
  echo "number confs = $number"
  echo "energy threhold = $energy"
...
#  echo "$smilist"
  ...
  foreach file (`ls prot-taut_split_????????`)
     ...
     #/home/baliuste/zzz.programs/corina/corina -i t=smiles -o t=mol2 -d rc,flapn,de=$energy,mc=$number,wh $newname.smi $newname.mol2
     corina -i t=smiles -o t=mol2 -d rc,flapn,de=$energy,mc=$number,wh $newname.smi $newname.mol2
     #python /home/baliuste/zzz.github/teb_scripts_programs/zzz.scripts/separate_mol2_more10000_mod.py $newname.mol2 ${newname}_split
     python ${TEBSCRIPTS}/zzz.scripts/separate_mol2_more10000_mod.py $newname.mol2 ${newname}_split

     foreach file (`ls *${newname}_split*.mol2`)
         echo $file
         set fileroot = ${file:r}
         cp $newname.smi $fileroot.smi
         ln -s $newname.name.txt $fileroot.name.txt
     end
     #exit
     @ count = $count + 1
  end
