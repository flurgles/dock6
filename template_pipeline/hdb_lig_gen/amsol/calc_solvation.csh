...
# modified by Trent E. Balius, FNLCR Dec 2022.  Add parameter to turn on antechamber, defult is that it will be turned off.  

set DOCK6BASE = ${DOCK_HOME}

  if ($#argv == 1) then
      echo "Antechamber will not be used.  add  --antechamber to turn on antechamber"
      echo "mol2file =  $1"
      set ante = "No"
  else if ($#argv == 2) then
      if ($2 != "--antechamber") then
         echo "the second argument  must be  --antechamber"
         exit
      endif
      set ante = "Yes"
      echo "Antechamber will be tried.  "
      echo "mol2file =  $1"
  else
